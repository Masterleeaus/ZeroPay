...
class MakePaymentController extends Controller
{
            ...
            $statusInfo = [
                ...
                if($item->attribute == payment_gateway_const()::SEND){
                    return[
                        ...
                        'recipient_received' => getAmount(@$item->details->recipient_amount,get_wallet_precision()).' '.get_default_currency_code(),
                    ...
                    return[
                        ...
                        'recipient_received' => getAmount(@$item->request_amount,get_wallet_precision()).' '.get_default_currency_code(),
                        'current_balance' => getAmount(@$item->available_balance,get_wallet_precision()).' '.get_default_currency_code(),
                        'status' => @$item->stringStatus->value ,
                        'date_time' => @$item->created_at ,
                        'status_info' =>(object)@$statusInfo ,
    ...
    public function confirmedPayment(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('user_id',$userWallet->user->id,PaymentGatewayConst::TYPEMAKEPAYMENT,$userWallet->currency,$amount,$makePaymentCharge,PaymentGatewayConst::SEND);
        }catch(Exception $e){
        ...
        try{
            ...
            $notifyDataSender = [
               ...
               'title'  => __("Make Payment to")." @" . @$receiver->username." (".@$receiver->email.")",
                'request_amount'  => getAmount($amount,get_wallet_precision()).' '.get_default_currency_code(),
                'payable'   =>  getAmount($payable,get_wallet_precision()).' ' .get_default_currency_code(),
                'charges'   => getAmount( $total_charge,get_wallet_precision()).' ' .get_default_currency_code(),
                'received_amount'  => getAmount( $recipient,get_wallet_precision()).' ' .get_default_currency_code(),
                'status'  => __("success"),
              ];
